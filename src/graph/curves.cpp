...
csg::Curve::Curve(const csc::Float2 min, const csc::Float2 max, const boost::optional<std::vector<CurvePoint>> points) : _min{ min }, _max{ max }
{
	if (points) {
		...
		this->points.push_back(csg::CurvePoint{ min, CurveInterp::CUBIC_HERMITE });
		this->points.push_back(csg::CurvePoint{ max, CurveInterp::CUBIC_HERMITE });
	}
	sort_points();
}
...
float csg::Curve::eval_point(const float input) const
{
	...
	if (p2_index.has_value() == false) {
		return points[points.size() - 1].pos.y;
	}

	const csc::Float2 p1{ points[*p1_index].pos };
	const csc::Float2 p2{ points[*p2_index].pos };
	const csg::CurveInterp p1_interp{ points[*p1_index].interp };
	...
	const csc::Float2 p0{ gen_p0_pos(points, *p1_index, *p2_index) };
	const csc::Float2 p3{ gen_p3_pos(points, *p1_index, *p2_index) };
...
size_t csg::Curve::create_point(const float x)
{
	...
	for (size_t i = 0; i < points.size(); i++) {
		if (points[i] == new_point) {
			...
			if (i > 0 && points[i - 1].interp == csg::CurveInterp::CUBIC_HERMITE) {
				interp = csg::CurveInterp::CUBIC_HERMITE;
			}
			if (i + 1 < points.size() && points[i + 1].interp == csg::CurveInterp::CUBIC_HERMITE) {
				interp = csg::CurveInterp::CUBIC_HERMITE;
			}
			points[i].interp = interp;
			return i;
		}
	}
	assert(false);
...
size_t csg::Curve::move_point(const size_t index, const csc::Float2 new_pos)
{
	...
	points[index].pos = valid_pos.clamp(new_pos);
	const CurvePoint changed_point{ points[index] };
	sort_points();

	// Find and return the point's new index
	for (size_t i = 0; i < points.size(); i++) {
		if (changed_point == points[i]) {
			return i;
		}
	}
	assert(false);
...
void csg::Curve::set_interp(const size_t index, const CurveInterp new_interp)
{
	...
	points[index].interp = new_interp;
}
...
bool csg::Curve::similar(const Curve& other, const float margin) const
{
	...
	for (size_t i = 0; i < points.size(); i++) {
		if (points[i].interp != other.points[i].interp) {
			return false;
		}
		if (points[i].pos.similar(other.points[i].pos, margin) == false) {
			return false;
		}
	}
...
void csg::Curve::sort_points()
{
	...
	std::sort(points.begin(), points.end(), lt_x);
}
